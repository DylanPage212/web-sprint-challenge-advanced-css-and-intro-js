...
/*
  for(let i = 0; i < array.length; i++) {
    let birthdate = parseInt(array[i].years, 10);
    if(age =  )
  }
}

I have no idea how to split up the dates into numbers that can be individually evaluated and already googled stuff for 30 minutes so I'm going
to just move on.
*/
...
function lotsOfArt(array){
  ...
  for (let i = 0; i < array.length; i++) {
    if(array[i].paintings > 100) {
      paintingsArray.push(array[i].name)
    }
  }
